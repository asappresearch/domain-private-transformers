...
TB_METRICS_TO_REPORT = ["ppl", "eval_losses"]
...
class Trainer:
    ...
    def __init__(
        ...
        train_redacted_dataset: Optional[Dataset] = None,
        ...
        **kwargs,
        ...
        self.train_redacted_dataset = train_redacted_dataset
    ...
    def train(self, model_path: Optional[str] = None, **kwargs):
        ...
        if self.args.train_redaction_schedule:
            train_redacted_dataloader = self.get_train_dataloader(self.train_redacted_dataset)
        ...
        # Log args before training
        all_args = {
            **self.args.__dict__,
            **self.data_args.__dict__,
            **self.model_args.__dict__,
            **self.privacy_args.__dict__,
        }
        utils.jdump(
            all_args,
            os.path.join(self.args.output_dir, "argparse.json"),
            default=lambda x: str(x),
        )

        # Save args in tensorboard
        self.tb_writer.add_text("args", utils.jdumps(all_args, default=lambda x: str(x)))
        ...
        for epoch in range(epochs_trained, int(np.ceil(num_train_epochs))):
            ...
            if self.args.train_redaction_schedule:
                if (
                    isinstance(train_redacted_dataloader, DataLoader)
                    and isinstance(train_redacted_dataloader.sampler, DistributedSampler)
                ):
                    train_redacted_dataloader.sampler.set_epoch(epoch)

                # Initialize the iterator
                train_redacted_iterator = iter(train_redacted_dataloader)
            ...
            # both model.zero_grad and model.original_zero_grad?
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                using_redaction = False
                if self.args.train_redaction_schedule:
                    # With probability p sample from redacted data
                    p = 0.0
                    if self.args.train_redaction_schedule == "step":
                        # Step schedule
                        p = (self.args.max_steps - self.global_step) / self.args.max_steps
                        p = float(int(p * 10.0)) / 10.0  # round to the nearest tenth
                    elif self.args.train_redaction_schedule == "linear":
                        p = (self.args.max_steps - self.global_step) / self.args.max_steps
                    elif self.args.train_redaction_schedule == "exp_concave":
                        p = 1.0 - np.exp((self.args.train_redaction_exp_scale / self.args.max_steps) * (self.global_step + 1 - self.args.max_steps))
                    elif self.args.train_redaction_schedule == "exp_convex":
                        p = np.exp(-self.args.train_redaction_exp_scale * self.global_step)

                    # if np.random.rand(1)[0] < p:
                    #     # TODO: index with step instead??
                    #     try:
                    #         inputs = next(train_redacted_iterator)
                    #     except StopIteration:
                    #         # Restart the iterator
                    #         train_redacted_iterator = iter(train_redacted_dataloader)
                    #         inputs = next(train_redacted_iterator)
                    #     using_redaction = True
                    # iterators should stay in sync with this implementation
                    try:
                        inputs_redacted = next(train_redacted_iterator)
                    except StopIteration:
                        # Restart the iterator
                        train_redacted_iterator = iter(train_redacted_dataloader)
                        inputs_redacted = next(train_redacted_iterator)
                    ...
                    p = 0.0
                self.tb_writer.add_scalar("train_redacted/sampling_p", p, self.global_step)

                do_loss_backward_in_step = (
                    self.privacy_args.non_private
                    or using_redaction
                )
                losses = self.training_step(
                    model, inputs, do_loss_backward=do_loss_backward_in_step
                )
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if self.privacy_args.non_private:
                        ...
                        self.optimizer.step()
                    elif using_redaction:
                        # Use original nonDP optimizer if redacted data
                        torch.nn.utils.clip_grad_norm_(model.parameters(), self.args.max_grad_norm)
                        self.optimizer.original_step()
                    else:
                        # private training with no redaction, use DP optimizer
                    ...
                    # cases for model.zero_grad and model.original_zero_grad
                    # based on privacy_args.non_private and using_redaction?
                    model.zero_grad(set_to_none=True)
                    # if using_redaction:
                    #     model.original_zero_grad(set_to_none=True)
    ...
    def log(self, logs: Dict[str, float], iterator: Optional[tqdm] = None) -> None:
        ...
        if "epoch" not in logs:
            logs["epoch"] = self.epoch if self.epoch is not None else 0
        if "global_step" not in logs:
            logs["global_step"] = self.global_step if self.global_step is not None else 0
        ...
        if self.tb_writer is not None:
            for split_name in ("train", "val", "eval"):
                ...
                split_metrics = output[split_name]["model"]
                for metric_name, v in split_metrics.items():
                    if metric_name not in TB_METRICS_TO_REPORT:
                        continue

                    self.tb_writer.add_scalar(
                        f"{split_name}/{metric_name}", v, global_step
                    )
    ...
    def evaluate(
        self,
        epoch: Optional[Union[int, float, str]] = None,
        eval_dataset: Optional[Dataset] = None,
        ...
        Args:
            epoch: `self.epoch` used if this is None
            eval_dataset: Evaluation dataset, using self.eval_dataset if this is None
            log_results: Store the results in `self.log_history` and print to stdout.

        Returns:
            A dictionary containing the evaluation loss and the potential metrics computed from the predictions.
        """

        metrics = {"epoch": epoch if epoch is not None else self.epoch}
        if self.global_step is not None:
            metrics = {"global_step": self.global_step}
        if hasattr(self.optimizer, "param_groups"):
            metrics["lr"] = [pg["lr"] for pg in self.optimizer.param_groups]

        train_sampler = self._get_train_sampler(self.train_dataset, shuffle=False)  # Don't shuffle during evaluation!
    ...
    def generate_and_write_to_file(self):
        ...
        stop_tokens = constants.get_stop_tokens(self.data_args.task_mode)
        # bad words should not be generated
        bad_words: Optional[List[str]] = None
        if constants.REDACTED in self.tokenizer.get_vocab():
            bad_words = [constants.REDACTED]
            if self.args.decoding_allow_redaction_token:
                bad_words.remove(constants.REDACTED)
        ...
        for this_model, this_model_tag in utils.zip_(models, model_tags):
            kwargs = dict(
                ...
                bad_words=bad_words,
                decoding_type=self.args.decoding_type,
                temperature=self.args.decoding_temperature,
                top_k=self.args.decoding_top_k,
                top_p=self.args.decoding_top_p,
            ...
            for split in ("train", "val", "eval"):
                if f"{split}_prompts" not in self.generation_stuff:
                    continue
                prompt_dataset = self.generation_stuff[f"{split}_prompts"]
                prompt_dataset = prompt_dataset[:self.args.decoding_max_generations]
                generations, prompts = decoding_utils.generate(
                ...
                output = [json.dumps({"context": prompt, "gen": gen})
                          for prompt, gen in zip(prompts, generations)]
                with open(generations_path, 'w') as f:
                    f.writelines([line + '\n' for line in output])
                logger.debug(f"Wrote generations to {generations_path}")
    ...
    def prediction_loop(
        ...
        for record_key, record_value in records.items():
            ...
            this_record["ppl"] = np.exp(this_record["eval_losses"])
        metrics = records
